...
# config logging
log.basicConfig(format='%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S', level=log.DEBUG)
...
if options.diss_path:
  diss_path = str(options.diss_path.strip())
    ...
    log.info("Le chemin de l espace de dissemination est manquant")
    exit(1)

# checking mapsmos executable
if not os.path.exists('mapsmos.py'):
    print "L executable mapsmos.py n existe pas"
    log.info("L executable mapsmos.py n existe pas")
    exit(1)
...
if not os.path.exists(diss_path):
  ...
  log.info("Erreur le chemin de dissemination " + diss_path + " n existe pas")
  exit(1)
else:
  ...
  if not os.path.exists(ftp_diss_path):
    ...
    log.info("Le repertoire de dissemination FTP " + ftp_diss_path + " n existe pas")
    exit(1)

  log.info("Nettoyage des cartes")
  map_diss_path = diss_path + os.sep + "MAP"
  log.debug("diss_path : " + map_diss_path)
  ...
  if not os.path.exists(map_diss_path):
    ...
    log.info("LOOP through MAP tree")
    ...
    for root, directories, filenames in os.walk(map_diss_path):
      for filename in filenames:
        ...
        log.debug("ftp_dir : " + ftp_dir)

        ftp_file = None
        file_to_match = filename.split('.')[0]

        log.debug("file to match : " + file_to_match)
        ...
        for loop_file in os.listdir(ftp_dir):
          if loop_file.startswith(file_to_match):
            ...
            break
        if ftp_file is None:
          log.info("Le produit associee a la carte " + filename + " n existe pas")
          ...
          while curr_dir != map_diss_path:
            if os.path.exists(curr_dir) and len(os.listdir(curr_dir)) == 0:
              os.rmdir(curr_dir)
              log.info("Repertoire vide supprime : " + curr_dir)
              curr_dir = os.path.dirname(curr_dir)
            else:
              break

  log.info("Generation des cartes")
      ...
      if (filename.endswith('.tgz') or filename.endswith('.zip')
        ...
        for prd in mapsmos_prd:
          ...
          if prd in filename:
            ...
            log.debug("map_dir : " + map_dir)

            map_file = None
            if os.path.exists(map_dir):
              file_to_match = filename.split('.')[0]

              log.debug("file_to_match : " + file_to_match)

              for loop_file in os.listdir(map_dir):
                if loop_file.startswith(file_to_match):
                  map_file = loop_file
                  log.info("La carte associee au produit " + filename + " existe deja")
                  break

            if map_file is None:
              log.debug( "map_file is None...")
              ...
              if file_to_process:
                ...
                log.debug("file to process...")
                try:
                  mapsmos.mapsmos(file_to_process, map_dir)
                  log.info("succes de la generation de la carte a partir du produit " + os.path.basename(file_to_process))
                except Exception as err:
                  log.info("echec de la generation de la carte a partir du produit " + os.path.basename(file_to_process) +
              ...
              break
log.info("Fin de la generation des cartes")
# cleaning
os.rmdir(tmpdir)
exit(0)
